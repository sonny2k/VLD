...
router.get('/:id', verifyToken, async(req, res) => {
    try {
        ...
        return res.status(400).json({success: false, message: 'Người dùng không có quyền cập nhật profile này'})
...
router.put('/:id', verifyToken, async(req, res) => {
    const {bloodtype, height, weight, pastmedicalhistory, drughistory, familyhistory} = req.body
    ...
    try {
        ...
        const profileupdatecondition = { _id: req.params.id, account: req.accountId }
        updatedUser = await User.findOneAndUpdate(profileupdatecondition, updatedUser, {new: true})
...
router.post('/changepassword', verifyToken, async(req, res) => {
    ...
    if (!passwordvalid)
    return res.status(400).json({success: false, message: 'Sai mật khẩu hiện tại'})
